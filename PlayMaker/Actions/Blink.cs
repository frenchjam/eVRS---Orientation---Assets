...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class Blink : FsmStateAction
	{
		...
		[Tooltip("Signal ON event to EEG recorder.")]
		public FsmString eegEventOn;

		[Tooltip("Signal OFF event to EEG recorder.")]
		public FsmString eegEventOff;
		...
		void UpdateBlinkState(bool state)
		{
			...
			if ( eegObject &&  state && !(0 == "".CompareTo ( eegEventOn.ToString () ) ) ) eegObject.SendMarker( eegEventOn.ToString () );
			if ( eegObject && !state && !(0 == "".CompareTo ( eegEventOff.ToString() ) ) ) eegObject.SendMarker( eegEventOff.ToString() );
