...
// Based on the FollowPath.cs from Itween
...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class FollowPath : iTweenFsmAction
	{
		[RequiredField] 
		...
		void DoiTween()
		{
			...
			if (pathName.Value != "")
			{
				if(reverse.Value)
				{
					hash.Add("path", MotionPath.GetPathReversed(pathName.Value)) ;
				}else{
					hash.Add("path", MotionPath.GetPath(pathName.Value)) ;
				}	
