...
// See http://www.blackbeltcoder.com/Articles/strings/a-sscanf-replacement-for-net */
...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("ScanFormatted, c# version of sscanf")]
    public class ScanFormatted : FsmStateAction
    {
        [RequiredField]
        [Tooltip("The string to scan")]
        public FsmString stringtoScan;

        [RequiredField]
        [Tooltip("A sequence that begins with a percent sign (%) to signify a format specifier, or field, that will be parsed and stored in a variable. A format specifier has the following form: %[*][width][modifiers]type. See Code.")]
        public FsmString modifier;

        [Tooltip("The %b variables read.")]
        public FsmBool[] boolResult;
        [Tooltip("The %i variables read.")]
        public FsmInt[] intResult;
        [Tooltip("The %f variables read.")]
        public FsmFloat[] floatResult;
        [Tooltip("The %s variables read.")]
        ...
        public override void Reset()
        {
            ...
            modifier = null;
            everyFrame = false;
            boolResult = new FsmBool[1];
            intResult = new FsmInt[1];
            floatResult = new FsmFloat[1];
            stringResult = new FsmString[1];
        ...
        {
            if (Parse(stringtoScan.Value, modifier.Value) > 0)
            {
                try
                {
                    int b = 0;
                    ...
                    foreach (var v in Results)
                    {
                        TypeSwitch.Do(
                            v,
                            TypeSwitch.Case<float>(x => addFloat(x, f++)),
                            TypeSwitch.Case<int>(x => addInt(x, i++)),
                            TypeSwitch.Case<string>(x => addString(x, s++)),
                            TypeSwitch.Case<bool>(x => addBool(x, b++)),
                            TypeSwitch.Default(() => Debug.Log("Unparsable: " + v.ToString()))
                            );
                    }
        ...
        private void addFloat(float v, int index)
        {
            floatResult[index].Value = v;
        }
        private void addBool(bool v, int index)
        {
            boolResult[index].Value = v;
        }
        private void addInt(int v, int index)
        {
            intResult[index].Value = v;
        }
        private void addString(string v, int index)
        {
            stringResult[index].Value = v;
        }
