	...
	{ 
				...
				if (mode == DriveMode.rotate || (mode == DriveMode.translate && (Input.GetKey (KeyCode.LeftAlt) || Input.GetKey (KeyCode.RightAlt)))) {
						...
						if (Input.GetKey (left)) 
								this.transform.Rotate (j_vector, - rotateSpeed * Time.deltaTime, Space.Self);
						if (Input.GetKey (right))
							this.transform.Rotate (j_vector, rotateSpeed * Time.deltaTime, Space.Self);
						if (Input.GetKey (up))
							this.transform.Rotate (i_vector, - rotateSpeed * Time.deltaTime, Space.Self);
						if (Input.GetKey (down))
							this.transform.Rotate (i_vector, rotateSpeed * Time.deltaTime, Space.Self);	
						// Roll around the current heading.
						if (Input.GetKey (inward))
							this.transform.Rotate (k_vector, rotateSpeed * Time.deltaTime, Space.Self);
						if (Input.GetKey (outward))
							this.transform.Rotate (k_vector, - rotateSpeed * Time.deltaTime, Space.Self);
						
				} else {
						if (Input.GetKey (left) )
								this.transform.Translate (- translateSpeed * Time.deltaTime * i_vector, Space.Self);
						if (Input.GetKey (right) )
							this.transform.Translate (translateSpeed * Time.deltaTime * i_vector, Space.Self);
						if (Input.GetKey (up)) 
							this.transform.Translate (translateSpeed * Time.deltaTime * j_vector, Space.Self);
						if (Input.GetKey (down) )
							this.transform.Translate (- translateSpeed * Time.deltaTime * j_vector, Space.Self);
						if (Input.GetKey (inward)) 
							this.transform.Translate (- translateSpeed * Time.deltaTime * k_vector, Space.Self);
						if (Input.GetKey (outward) )
							this.transform.Translate (translateSpeed * Time.deltaTime * k_vector, Space.Self);
